surfaceNormals<-function(x,indexAlpha = 1,display = FALSE, col=3, scale = 1, ...){
	as3d<-x
	tetra<-as3d$tetra
	...
	if (class(indexAlpha)=="character" & (indexAlpha=="ALL" | indexAlpha=="all") )
	...
	for ( iAlpha in indexAlpha)
	{
		...
		retour<-.C("triangleNormals",as.integer(tr),dim(tr)[1],as.integer(te),dim(te)[1],as.numeric(x),dim(x)[1],normMat,middlePoint)
		...
		if(display)
		{
			...
			for(ii in 1 :dim(normMat)[1] )
			{
				...
				segment[2*ii,]  =middlePoint[ii,] + scale * normMat[ii,]
			}
			rgl.open()
			...
			normals.obj <- c(normals.obj,  list(list("normals"=normMat,"centers"=middlePoint)))
